...
def build_tree(sl, n, level = 0):
    if len(sl) <= n:
        ntot = sum([si[1] for si in sl])
        return ("L%d" % level, ntot, sl)
    # more
...
if __name__ == "__main__":
    sl = ["test:{0}".format(i) for i in range(17)]
    nodes = [[si, 1, []] for si in sl]
    stree = build_tree(nodes, 8)
    #print stree
    #output_tree(stree, 0)
